...
namespace geometry
{
    ...
    bool is_intersecting(point<T> a, point<T> b, point<T> c, point<T> d)
    {
        ...
        {
            if ((std::abs(ac & ad) <= std::abs((ac & ab) + (ab & ad))) && (0 <= (ac & ab) * (ab & ad)) && (0 <= (ac & ab) * (ac & ad)) && (0 <= (ab & ad) * (ac & ad)))
...
int main()
{
    ...
    std::cin >> x >> y;
    geometry::point<long long> a(x, y);
 
    std::cin >> x >> y;
    geometry::point<long long> b(x, y);
 
    std::cin >> x >> y;
    geometry::point<long long> c(x, y);
 
    std::cin >> x >> y;
    geometry::point<long long> d(x, y);
 
    if (geometry::is_intersecting(a, b, c, d)) 
    {
        std::cout << "YES";
    } else {
        std::cout << "NO";
